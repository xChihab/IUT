...
public class Film {
	...
	public String toString() {
		return String.format(
				"Titre: %-80s, Année: %-4d, Genres: %-35s, Durée: %-5d min, Pays: %-30s, Langue: %-30s, Réalisateur: %-30s, Scénariste: %-40s, Acteurs: %-100s, Description: %-400s, Votes: %-10d, Moyenne des votes: %-5.1f",
	...
	public static void filtreLineaire(List<Film> films, String critere, Object valeur) {
	        ...
	        switch (critere.toLowerCase()) {
	            case "titre":
	                if (valeur instanceof String) {
	                    correspond = film.getTitre().toLowerCase().contains(((String) valeur).toLowerCase());
	                }
	                break;

	            case "annee":
	                if (valeur instanceof Integer) {
	                    correspond = film.getAnnee() == (int) valeur;
	                }
	                break;

	            case "genre":
	                if (valeur instanceof String) {
	                    correspond = film.getGenres().stream()
	                            .anyMatch(genre -> genre.equalsIgnoreCase((String) valeur));
	                }
	                break;

	            case "duree":
	                if (valeur instanceof Integer) {
	                    correspond = film.getDuree() == (int) valeur;
	                }
	                break;

	            case "pays":
	                if (valeur instanceof String) {
	                    correspond = film.getPays().equalsIgnoreCase((String) valeur);
	                }
	                break;

	            case "langue":
	                if (valeur instanceof String) {
	                    correspond = film.getLangue().equalsIgnoreCase((String) valeur);
	                }
	                break;

	            case "realisateur":
	                if (valeur instanceof String) {
	                    correspond = film.getRealisateur().equalsIgnoreCase((String) valeur);
	                }
	                break;

	            case "scenariste":
	                if (valeur instanceof String) {
	                    correspond = film.getScenariste().equalsIgnoreCase((String) valeur);
	                }
	                break;

	            case "acteurs":
	                if (valeur instanceof String) {
	                    correspond = film.getActeurs().stream()
	                            .anyMatch(acteur -> acteur.equalsIgnoreCase((String) valeur));
	                }
	                break;

	            case "nombrevotes":
	                if (valeur instanceof Integer) {
	                    correspond = film.getNombreVotes() == (int) valeur;
	                }
	                break;

	            case "note":
	                if (valeur instanceof Double) {
	                    correspond = film.getMoyenneVotes() == (double) valeur;
	                }
	                break;
